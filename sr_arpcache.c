...
void handle_arpreq(struct sr_instance* sr, struct sr_arpreq* req){
    ...
    if (difftime(now,req->sent)>1.0){
        if (req->times_sent >= 5){
            ...
            packet_len = sizeof(struct sr_ethernet_hdr)+sizeof(struct sr_arp_hdr)+4;
            ...
            for (if_now=if_list;if_now!=NULL;if_now=if_now->next){
                memcpy(a_hdr->ar_sha, if_now->addr, sizeof(if_now->addr));
                a_hdr->ar_sip = if_now->ip;
                sr_send_packet(sr,packet,packet_len,if_now->name);
                req->sent = now;
                req->times_sent++;
            }
        }
